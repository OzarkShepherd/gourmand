...
import gourmand.gtk_extras.WidgetSaver as WidgetSaver
from gourmand.defaults import lang as defaults
from gourmand.gtk_extras.mnemonic_manager import MnemonicManager
from gourmand.gtk_extras.pageable_store import PageableViewStore
from gourmand.gtk_extras.validation import NumberEntry
...
class NutritionUSDAIndex:
    ...
    PACKAGED_FOODS = ["Soups, Sauces, and Gravies", "Baked Products", "Meals, Entrees, and Sidedishes", "Fast Foods", "Baby Foods"]
    ...
    def __init__(self, rd, prefs, widgets):
        ...
        WidgetSaver.WidgetSaver(self.usdaSearchAsYouTypeToggle, self.prefs.get("sautTog", {"active": True}), ["toggled"])
...
class NutritionInfoDruid(GObject.GObject):
    """A druid (or "wizard") to guide a user through helping Gourmand
    ...
    def __init__(self, nd, prefs, rec=None, in_string=""):
        self.ui = Gtk.Builder()
        self.ui.add_from_string(get_data("gourmand", "ui/nutritionDruid.ui").decode())
        ...
        WidgetSaver.WindowSaver(self.ui.get_object("window"), self.prefs.get("nutritionDruid", {}))
    ...
    def _setup_widgets_(self):
        ...
        self.widgets = [
            ...
            "editButton",
            # INFO PAGE
            "infoIngredientKeyLabel",
            "infoUSDALabel",
            "nutritionLabelBox",
            "infoDensityLabel",
            "infoOtherEquivalentsLabel",
        ...
        for widget_name in self.widgets:
            ...
            if widget_name != "notebook":
                self.controls.append(getattr(self, widget_name))
        self.usdaIndex = NutritionUSDAIndex(self.rd, prefs=self.prefs, widgets=[(w, getattr(self, w)) for w in self.widgets])
        ...
        self.changeIngKeyAction = SpecialAction(
            highlight_widgets=[
                self.ingKeyEntry,
                self.applyKeyButton,
            ],
        ...
        self.changeUnitAction = SpecialAction(
            highlight_widgets=[
                self.fromUnitComboBoxEntry,
                self.saveUnitButton,
    ...
    def view_nutritional_info(self, *args):
        ...
        if nutalias.ndbno == 0:
            ...
            self.add_ingredients([(nutalias.ingkey, self.get_amounts_and_units_for_ingkey(nutalias.ingkey))])
        else:
            self.show_info_page(nutalias)
            self.path.append((self.show_info_page, [nutalias]))
    ...
    def set_density_info(self, nutalias):
        ...
        eutexts.sort()
        extra_units_text = "\n".join(eutexts)
        self.infoOtherEquivalentsLabel.set_text(extra_units_text or "None")
        others = self.rd.fetch_all(self.rd.nutritionconversions_table, ingkey=nutalias.ingkey)
        # other_label = "\n".join(["%s: %.1f g" % (conv.unit or "100 %s" % _("ml"), 1.0 / conv.factor) for conv in others])
        if others:
    ...
    def populate_custom_equivalents_table(self, equivalents):
        ...
        for c in self.infoCustomEquivalentsTable.get_children():
            self.infoCustomEquivalentsTable.remove(c)
            c.unparent()
        for n, eq in enumerate(equivalents):
            lab = Gtk.Label("%s: %.1f g" % (eq.unit or "No unit", 1.0 / eq.factor))
            rembut = Gtk.Button("C_hange")
            rembut.set_use_underline(True)
            rembut.connect("clicked", self.info_edit_equivalent, eq)
            self.infoCustomEquivalentsTable.attach(lab, 0, 1, n, n + 1)
            self.infoCustomEquivalentsTable.attach(rembut, 1, 2, n, n + 1)
    ...
    def set_nutritional_label(self, nutalias):
        if not hasattr(self, "nutritionLabel"):
            ...
            self.nutritionLabelBox.pack_start(self.nutritionAmountLabel, fill=0, expand=0)
        ...
        amount, unit = self.def_ingredient_amounts.get(nutalias.ingkey, (self.DEFAULT_AMOUNT, self.DEFAULT_UNIT))
        nutinfo = self.nd.get_nutinfo_for_inglist([MockObject(amount=amount, unit=unit, ingkey=nutalias.ingkey)], self.rd)
    ...
    def info_edit_usda_association(self, *args):
        ...
        self.edit_nutinfo(ingkey=self.info_nutalias.ingkey, desc=self.info_nutalias.desc)
        self.path.append((self.edit_nutinfo, [self.info_nutalias.ingkey, self.info_nutalias.desc]))
    ...
    def info_edit_equivalent(self, button, eq):
        ...
        self.setup_to_units()
        to_unit = cb.cb_set_active_text(self.toUnitCombo, "g")  # noqa: F841
        self.toAmountEntry.set_text(convert.float_to_frac(amt_in_grams, fractions=convert.FRACTIONS_ASCII))
        ...
        # event)
        if button:
            self.path.append((self.info_edit_equivalent, [None, eq]))
    ...
    def _setup_custom_box(self):
        ...
        for n, nutstuff in enumerate(NUT_LAYOUT):
            ...
            if show_percent:
                percent_entry = NumberEntry(default_to_fractions=False, decimals=0)
    ...
    def number_changed_cb(self, widget, name, percent_widget):
        ...
        if not v:
            return
        if self.changing_number_internally:
            return
        if percent_widget:
    ...
    def percent_changed_cb(self, widget, name, number_widget):
        if self.changing_percent_internally:
            return
        v = widget.get_value()
        if not v:
            return
    ...
    def apply_custom(self, *args):
        ...
        ndbno = self.nd.add_custom_nutrition_info(nutinfo)  # noqa: F841
    ...
    def set_from_unit(self, txt):
        ...
        if txt:
            ...
            self.fromUnit = txt
            curamt = " ".join([convert.float_to_frac(self.amount, fractions=convert.FRACTIONS_ASCII), self.fromUnit, self.ingkey])
        else:
            self.fromUnitLabel.set_text(self.ingkey + " (no unit)")
            self.fromUnit = ""
            curamt = convert.float_to_frac(self.amount, fractions=convert.FRACTIONS_ASCII) + " " + self.ingkey
        self.convertUnitLabel.set_markup(
            '<span weight="bold" size="larger">'
            + _("Convert unit for %s") % self.ingkey
            + "</span>"
            + "\n<i>"
            + _('In order to calculate nutritional information, Gourmand needs you to help it convert "%s" into a unit it understands.') % curamt
            + "</i>"
        )
    ...
    def apply_ingkey(self, *args):
        ...
        if self.rec:
            try:
                user_says_yes = de.getBoolean(
                    ...
                    sublabel=_("Change ingredient key from %(old_key)s to %(new_key)s everywhere or just in the recipe %(title)s?")
                    % {"old_key": self.ingkey, "new_key": key, "title": self.rec.title},
        ...
        if self.rec and user_says_yes:
            self.rd.update_by_criteria(self.rd.ingredients_table, {"ingkey": self.ingkey, "recipe_id": self.rec.id}, {"ingkey": key})
        else:
            self.rd.update_by_criteria(self.rd.ingredients_table, {"ingkey": self.ingkey}, {"ingkey": key})
        old_key = self.ingkey
        self.set_ingkey(key)
    ...
    def save_unit_cb(self, *args):
        ...
        if self.rec and de.getBoolean(
            label=_("Change unit"),
            sublabel=_("Change unit from %(old_unit)s to %(new_unit)s for all ingredients %(ingkey)s or just in the recipe %(title)s?")
            % {"old_unit": old_from_unit, "new_unit": from_unit, "ingkey": self.ingkey, "title": self.rec.title},
            custom_no=_("Change _everywhere"),
            custom_yes=_("_Just in recipe %s") % self.rec.title,
        ):
            self.rd.update_by_criteria(self.rd.ingredients_table, {"ingkey": self.ingkey, "unit": old_from_unit, "recipe_id": self.rec.id}, {"unit": from_unit})
        else:
            self.rd.update_by_criteria(self.rd.ingredients_table, {"ingkey": self.ingkey, "unit": old_from_unit}, {"unit": from_unit})
    ...
    def apply_density(self):
        self.nd.set_density_for_key(self.ingkey, self.custom_density)
        for c in self.densityBox.get_children():
            c.hide()
    ...
    def check_next_amount(self):
        ...
        existing_conversion_fudged = existing_conversion and (not self.nd.get_conversion_for_amt(amount, unit, self.ingkey, fudge=False))
        if existing_conversion_fudged:
            ...
            self.path.append((self.edit_units, [amount, unit, self.ingkey, self.amount_index]))

    def edit_units(self, amount, unit, ingkey, indx=None):
        self.set_ingkey(ingkey)
        self.set_from_unit(unit)
        if indx is not None:
            self.amount_index = indx
        self.fromAmountEntry.set_text(convert.float_to_frac(amount, fractions=convert.FRACTIONS_ASCII))
        self.toAmountEntry.set_text(convert.float_to_frac(amount, fractions=convert.FRACTIONS_ASCII))
        self.goto_page_unit_convert()
    ...
    def apply_cb(self, *args):
        ...
        if page == self.NUT_PAGE:
            ...
            if not self.custom_factor:
                de.show_message(_("To apply nutritional information, Gourmand needs a valid amount and unit."))
                return
            self.apply_custom()
            self.check_next_amount()
    ...
    def goto_page_key_to_nut(self):
        for b in [self.applyButton, self.ignoreButton]:
            b.show()
        for b in [self.editButton]:
            b.hide()
    ...
    def goto_page_info(self):
        for b in [self.editButton, self.applyButton, self.ignoreButton]:
            b.hide()
