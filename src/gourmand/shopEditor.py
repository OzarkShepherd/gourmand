...
class ShopEditor:
    ...
    def __init__(self, rd=db.recipeManager(), rg=None):
        ...
        # setup WidgetSavers
        self.rg.conf.append(
            WidgetSaver.WidgetSaver(self.searchAsYouTypeToggle, self.prefs.get("sautTog", {"active": self.searchAsYouTypeToggle.get_active()}), ["toggled"])
        )
        self.rg.conf.append(WidgetSaver.WidgetSaver(self.regexpTog, self.prefs.get("regexpTog", {"active": self.regexpTog.get_active()}), ["toggled"]))

    def dont_ask_cb(self, widget, *args):
        self.dont_ask = widget.get_active()

    def sort_model_fun(self, model, iter1, iter2, data):
        ...
        if not str and self.search_by == self.KEY_COL:
            ...
            if cat in self.cat_to_key:
                for itm in self.cat_to_key[cat]:
                    if self.use_regexp:
                        if re.search(self.search_string, itm):
                            return True
                    elif itm.find(self.search_string) >= 0:
                        return True
        if self.use_regexp:
            if re.search(self.search_string, str):
                return True
        else:
    ...
    def tree_edited(self, renderer, path_string, text, n, head):
        ...
        if n == self.KEY_COL and key == text:
            return
        if n == self.ITEM_COL and item == text:
            return
        ...
        if not self.dont_ask:
            ...
            if item:
                ...
                msg += ' from "%s" ' % key
            msg += ' to "%s"' % text
            if not de.getBoolean(label=msg, dont_ask_cb=self.dont_ask_cb, dont_ask_custom_text="Don't ask me before changing keys and items."):
                return
